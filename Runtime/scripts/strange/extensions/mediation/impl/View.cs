...
namespace strange.extensions.mediation.impl
{
	public class View : MonoBehaviour, IView
	{
		...
		{
			var msg = "A view was added with no context. Views must be added into the hierarchy of their ContextView lest all hell break loose.";

			var contextView = FindContext(view);
			if (!contextView)
			{
			...
			if (contextView != null)
			{
				...
				if (context == null)
				{
					Debug.LogError($"ContextView {contextView} has no context for {view}");
				}
				else
				{
					bool success = true;
		...
		protected virtual ContextView FindContext(MonoBehaviour view)
		{
			const int loopMax = 100;
			var loopLimiter = 0;
			var trans = view.gameObject.transform;
			while (trans.parent && loopLimiter < loopMax)
			{
				loopLimiter++;
				trans = trans.parent;
